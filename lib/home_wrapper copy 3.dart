...
import 'package:intl/intl.dart'; // ✅ อย่าลืมเพิ่มการ import intl ในไฟล์นี้ด้วยนะครับ
...
// ------------------------------------------------------------------
// ✅ 4. หน้า BetTypeScreen (หน้าเลือกประเภทหวย พร้อมระบบเช็คเวลา)
// ------------------------------------------------------------------
class BetTypeScreen extends StatelessWidget {
  const BetTypeScreen({super.key});

  // ✅ ฟังก์ชันเช็คว่าหวยเปิดหรือปิดตามเวลาปัจจุบัน
  bool _isLottoOpen(String? openStr, String? closeStr) {
    if (openStr == null || closeStr == null)
      return true; // ถ้าไม่มีข้อมูลเวลา ให้เปิดไว้ก่อน

    try {
      final now = DateTime.now();
      final DateFormat timeFormat = DateFormat('HH:mm');

      // แปลงเวลาจาก String เป็น DateTime ของวันนี้
      ...
      // คืนค่า true หากเวลาปัจจุบันอยู่ระหว่างเปิดและปิด
      return now.isAfter(openTime) && now.isBefore(closeTime);
    } catch (e) {
      return false; // หากเกิดข้อผิดพลาดในการแปลงเวลา ให้ปิดไว้เพื่อความปลอดภัย
    }
  }
        ...
        builder: (context, snapshot) {
          ...
          return GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              // ✅ เช็คสถานะเปิด-ปิด
              bool isOpen = _isLottoOpen(data['openTime'], data['closeTime']);

              return AbsorbPointer(
                absorbing: !isOpen, // ✅ ถ้าปิดรับแทง จะบล็อกการคลิกทันที
                child: Opacity(
                  opacity: isOpen ? 1.0 : 0.6, // ✅ ถ้าปิดรับแทง ให้ Card จางลง
                  child: InkWell(
                    onTap: () {
                      Navigator.push(
                        context,
                    ...
                    child: Card(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // ✅ แสดงสถานะตามเวลาจริง
                          Text(
                            isOpen
                                ? "🟢 เปิดรับแทง"
                                : "🔴 ปิดรับแทง (${data['closeTime']})",
                            style: TextStyle(
